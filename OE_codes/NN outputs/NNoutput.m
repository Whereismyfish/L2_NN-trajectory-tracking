...
function [dy,tao_u,tao_r,e_u,e_r,dist1,dist3,x_d,y_d,Feu,Fer,x_e,y_e,psi_e,norm_Wu,norm_Wr,non1,non2] = NNoutput(t,y,m11,m22,m33,...
...
elseif tao_u>=-B_tao_u && tao_u<=B_tao_u
    act_tao_u = B_tao_u*tanh(tao_u/B_tao_u);
end
...
elseif tao_r>=-B_tao_r && tao_r<=B_tao_r
    act_tao_r = B_tao_r*tanh(tao_r/B_tao_r);
end
...
dy(4) = f_u + act_tao_u/m11 + dist1;
dy(5) = f_v + dist2;
dy(6) = f_r + act_tao_r/m33 + dist3;
% dy(24) = t*abs(x_e);% ITAE
% dy(25) = t*abs(y_e);
% dy(26) = t*abs(psi_e);
...
dy(27) = abs(tao_u);% MAI
dy(28) = abs(tao_r);
%%% modified on 2023-07-17

delta_taou = act_tao_u - tao_u;
delta_taor = act_tao_r - tao_r;
norm_Wu = norm(Wu);%%% modified on 2023-07-17
norm_Wr = norm(Wr);%%% modified on 2023-07-17

dy(29) = (u_d - y(29))/const_t;
diff_u_d = (u_d - y(29))/const_t;
dy(30) = (r_d - y(30))/const_t;
diff_r_d  = (r_d - y(30))/const_t;
non1 = delta_taou/m11 - diff_u_d;
non2 = delta_taor/m33 - diff_r_d;
